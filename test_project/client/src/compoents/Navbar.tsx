...
const Navbar = () => {
    ...
    const navItems = ["Market", 'Exchange', 'Transactions', 'Services', 'About', 'Contact']
    ...
    return (
        <nav className="w-full flex md:justify-center justify-between items-center"> 
            ...
            <ul className="text-white md:flex hidden list-nonde flex-row justify-between items-center flex-initial">
                {navItems.map((item, index) => (
                    <NavbarItem key={item + index} title={item} classProps="" />
                )
                )}
                <li className="bg-[#2952e3] py-2 px-7 mx-4 my-4 rounded-full cursor-pointer hover:bg-[#2546bd]">
                    Login
                </li>
            </ul>
